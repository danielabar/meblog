...
A recent example: I was working on a back office system where we were adding admin features and onboarding internal users for customer support. We needed structured audit logs — who did what, when, and to which record — rather than the scattered `Rails.logger.info` calls we'd been relying on. I knew [Fizzy](https://github.com/basecamp/fizzy), 37signals' open source Kanban tool, had an event tracking system worth studying. But I didn't want to spend a day reading through the entire Fizzy codebase just to understand one pattern.

I pointed Claude at Fizzy's `Event` model and the related files and asked it to explain the pattern. Within minutes, it identified the key architectural decisions:

- **Polymorphic `eventable` association** — any model can generate events by including an `Eventable` concern, which gives it a `track_event` method. The concern automatically prefixes actions with the model name (`card_published`, `card_closed`, `comment_created`), so event types are self-documenting without maintaining a separate enum or constant list.
- **A JSON `particulars` column** — rather than creating separate tables or columns for every piece of event metadata, Fizzy stores context-specific details (like `assignee_ids`, `old_title`, `new_board`) in a JSON column accessed through `store_accessor`. Flexible enough to capture different data for different event types, structured enough to query.
- **Separate `Description` class** — event descriptions are generated by a dedicated class that knows how to render human-readable sentences from event data, handling things like "You" vs. the creator's name depending on who's viewing. This keeps display logic out of the model entirely.
- **Webhook dispatch on create** — every event automatically triggers webhook delivery via `after_create_commit`, so external systems can subscribe to the activity stream.

We haven't adopted this pattern yet — it's still in the early brainstorming phase. But that's part of the point. You don't need to be mid-implementation to get value from this kind of analysis. When you're exploring an architectural direction and want to see how others have solved it, an AI assistant can compress what would normally be a half-day of reading and note-taking into a thirty-minute exploration.

And because the assistant also has access to *your* codebase, it can go further than just explaining the open source pattern — it can identify where the adaptation points are. Fizzy uses plain old Ruby objects and concerns to organize its event logic. Our codebase organizes business logic in service objects. The assistant can bridge that gap: here's the pattern, here's how your code is structured differently, and here's how you might reconcile the two. That kind of cross-codebase reasoning is something that's genuinely hard to do quickly on your own, especially when you're not yet familiar with the source project.

<aside class="markdown-aside">
This approach works especially well with 37signals' open source projects (Fizzy, Campfire, Writebook) because their code tends to be clean and well-structured, with patterns that are intentionally designed to be extractable. But the technique applies to any well-maintained open source codebase.
</aside>
...
This gets even harder when you're crossing technology boundaries. I'm a Rails developer. My day-to-day is a Rails monolith — ActiveRecord, PosgreSQL, RSpec, Sidekiq, the usual. But an upcoming initiative requires me to work across our company's main product, which is a .NET application, backed by MongoDB with an Angular SPA frontend. Different language, different framework, different database paradigm, different everything.
...
- MongoDB instead of PostgreSQL — no schema, no migrations, no `rails db:migrate`? "Adding a field to an entity just works. Old documents without that field return `null`. But also no rollback safety net." No foreign keys, no joins, no referential integrity at the database level — relationships are just ID strings that application code queries by convention. Coming from PostgreSQL where the database enforces constraints for you, that's a significant mental shift.

It also identified the biggest mindset shifts — no `rails console` equivalent for poking at the database, explicit service registration instead of convention-over-configuration, the hybrid AngularJS/Angular 18 frontend where some features are in `app/` (legacy JavaScript) and others in `ng-app/` (modern TypeScript).
...
When I asked Claude to review the README, its first observation was that the documentation didn't communicate what made the project genuinely valuable. The README talked about implementation details — YAML configuration, command-line flags, output tables — but it didn't explain *why* someone should care. It didn't explain that the simulator handles tax-aware RRSP withdrawals with reverse tax calculations, models CPP and OAS as interacting taxable income sources, or enforces RRIF mandatory withdrawal rules. It didn't mention that it uses Geometric Brownian Motion with fat-tailed shocks to stress-test plans against futures worse than history.
...
- For the search work: seven distinct indexing attempts — basic GIN indexes, partial indexes per content type, compound indexes using the btree_gin extension, specialized partial indexes — each with commit evidence showing what was tried and why it was insufficient. A deploy-revert-redeploy cycle where the first custom SQL implementation was pushed to production, found problematic, reverted, and reimplemented with a feature flag for A/B comparison. The progression from "maybe we can make the gem work with better indexes" to "the fundamental SQL structure is the problem" was visible right there in the commit timeline.

- For the architecture work: the iterative journey from a first experiment (server-rendering just the navigation bar) to discovering that server-rendered links break SPA routing, to building a bridge layer that intercepts clicks and routes them through Backbone, to the final "shell" pattern where a Backbone view acts as a container for server-rendered content fetched via AJAX. Each phase had its own ticket and commits showing the progression.
