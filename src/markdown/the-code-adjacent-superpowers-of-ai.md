...
- No `rails console` equivalent for exercising application code in a REPL — you're working without the interactive feedback loop Rails developers rely on for exploration and debugging.
- The Angular frontend was mid-migration from legacy AngularJS to modern Angular, with separate directory structures — `app/` for legacy JavaScript components and `ng-app/` for modern TypeScript — so I'd know which convention to follow when working in different parts of the frontend.
