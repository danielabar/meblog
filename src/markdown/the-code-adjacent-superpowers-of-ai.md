...
It also identified the biggest mindset shifts — no `rails console` equivalent for exercising application code in a REPL, explicit service registration instead of convention-over-configuration. It even detected that the Angular frontend was mid-migration from legacy AngularJS to modern Angular, pointing out the separate directory structures — `app/` for legacy JavaScript components and `ng-app/` for modern TypeScript — so I'd know which convention to follow when working in different parts of the frontend.
