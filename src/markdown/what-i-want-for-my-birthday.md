...
Seyfried then lays out an alternate hypothesis that better fits the observed behaviour of cancer cells: nearly every cancer cell has defective mitochondria. Healthy cells run on cellular respiration, using oxygen to burn fuel efficiently, the way a car engine burns gasoline. Cancer cells can't do that. Instead they fall back on fermentation, an older and cruder way of generating energy, closer to a campfire than an engine: it works, but it wastes most of the fuel it burns to get a smaller amount of usable energy out. That inefficiency is why cancer cells pull in many times more glucose than a healthy cell needs, even when oxygen is plentiful.
...
This isn't hypothetical. In 2024, the FDA and Health Canada approved [vorasidenib](https://www.fda.gov/drugs/resources-information-approved-drugs/fda-approves-vorasidenib-grade-2-astrocytoma-or-oligodendroglioma-susceptible-idh1-or-idh2-mutation), a drug that targets a specific IDH1 mutation, found mostly in slower-growing, lower-grade gliomas (not glioblastoma). It costs nearly 40K USD per month (roughly 478,000 dollars a year) and is taken indefinitely, until progression or dose toxicity.
...
**The protocol is hard:** KMT is nothing like the keto of recipe blogs and keto desserts. It is not for weight loss. In fact for some patients losing weight is undesirable, so the diet has to include far more fat than anyone naturally cooks with, just to hold weight steady. And the protocol involves more than dieting. Here are just a few critical details:
...
- **None of this works without daily monitoring:** Glucose and ketones need daily blood-meter checks, tracked against a target ratio (the GKI). Research points to a GKI between 1 and 2 (some studies go up to 3) as the range that puts the most pressure on the cancer, and simply cutting back on sugar and starchy carbs won't get you there.
- **Monitoring has a recurring cost:** The meter itself is a one-time purchase, but glucose strips, ketone strips, and lancets are all single-use, and purchased separately. The patient also needs to learn how to safely perform daily finger pricking. None of this is currently treated as a covered medical expense.
