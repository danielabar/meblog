...
The artificial time constraint creates a "race against the clock mentality".
...
TODO: Maybe also story about deployment causing critical functionality to be broken, and this was in the days before containerized deployments, and CI/CD systems, and easy rollbacks so the bug had to be investigated and resolved within hours.
...
Speaking from my own experience, I find it nearly impossible to think about a solution to a coding problem *and* talk out loud about it at the same time. I can however explain it in a written format, after the problem is solved, and generate educational materials such as internal engineering documentation or a [blog posts](../python-interview-question-in-ruby). However, by rejecting candidates for not being able to think out loud and code at the same time, it creates the impression that only one particular way of thinking is valued.
...
An argument in favour of the timed live coding round is that projects need to meet deadlines so companies need engineers that can code quickly.
...
Many of the above issues are of communication, and won't be resolved by hiring the fastest coder.
