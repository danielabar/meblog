...
Claude was also able to trace the listeners back to their source, which was a script named `wp-security-front-script.js`. This is shipped by [All-In-One Security (AIOS)](https://wordpress.org/plugins/all-in-one-wp-security-and-firewall/), which is a WordPress plugin that adds firewall rules, login lockout after failed attempts, and two-factor authentication. Copy Protection is an optional toggle in this plugin's settings. While not described on the official plugin's page, this [tutorial](https://www.webnots.com/wordpress-all-in-one-wp-security-and-firewall-plugin-tutorial/) shows that enabling Copy Protection will:
...
The `<style>` tag fixes selection. `user-select: none` is a CSS rendering rule the browser applies, which prevents text selection. So this snippet undoes that with a competing CSS rule using `!important`.
